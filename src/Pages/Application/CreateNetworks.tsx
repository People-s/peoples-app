...
function CreateNetworks() {
  ...
  const LensHubContract = new Contract(
    addresses["lensHub proxy"],
    LensHubInterface
  );

  //hooks
  const { send: createProfileSend } = useContractFunction(
    LensHubContract,
    "createProfile"
  );
  //   const { send: unPauseSend } = useContractFunction(
  //     LensHubContract,
  //     "setState"
  //   );
  const { send: whiteListProfileSend } = useContractFunction(
    LensHubContract,
    "whitelistProfileCreator"
  );
  const { send: whiteListFollowModuleSend } = useContractFunction(
    LensHubContract,
    "whitelistFollowModule"
  );
  const { send: setFollowModuleSend } = useContractFunction(
    LensHubContract,
    "setFollowModule"
  );
  ...
  function whiteListProfileCreator() {
    // hardcoded for  user
    return whiteListProfileSend(
      "0x7566DEa64772032Bc46aBa02Cf264f69BF75E161",
      true
    );
  }

  function whiteListFollowModule() {
    return whiteListFollowModuleSend(
      addresses["Required Currency / NFT Follow Module"],
      true
    );
  }
  ...
  function initializeFollowModule() {
    const data = defaultAbiCoder.encode(
      ...
      [NFTAddress, NFTAmount, currencyAddress, currencyAmount]
    );
    return setFollowModuleSend(
      1,
      addresses["Required Currency / NFT Follow Module"],
      data
    );
  ...
  return (
    <>
      ...
      <div>
        ...
        <Input
          onChange={(e) => {
            setHandleNetwork(e.target.value);
          }}
        ></Input>
      </div>
      <div>
